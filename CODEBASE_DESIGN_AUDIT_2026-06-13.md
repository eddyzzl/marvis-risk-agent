...
node --check marvis/static/js/*.js
...
# Successfully built marvis
```

已用 zipfile 确认 wheel 包含：

- `marvis/static/js/branding.js`
- `marvis/static/css/welcome.css`
- `marvis/static/js/api.js`
- `marvis/static/app.js`
- `marvis/static/styles.css`
...
- `marvis/static/index.html` 使用 `<script type="module" src="static/app.js?...">`。
- `marvis/static/app.js` 第 1 行开始导入 `./js/api.js`、`./js/dialogs.js`、`./js/polling.js`、`./js/render-agent.js`、`./js/render-metrics.js` 等模块。
- 当前存在 `marvis/static/js/*.js`。
- `pyproject.toml` 的 package-data 只有：

```toml
marvis = ["static/*", "static/brand/*", "static/pets/*", "static/pets/naitang/*", "static/pets/xiaojiu/*"]
```
...
3. 增加安装包 smoke test：
   - `python -m build`
   - 在临时 venv 安装 wheel。
   - 断言 `importlib.resources.files("marvis").joinpath("static/js/api.js").is_file()`。
   - 启动 FastAPI TestClient，GET `/`、`/static/app.js`、`/static/js/api.js` 都应返回 200。

**建议测试**

- 新增 `tests/test_package_data.py`，直接检查 `marvis/static/js/api.js` 在包资源中可见。
...
`marvis/api.py` 当前用 `_task_stop_reason_code()` 生成 payload 里的 `stopped` / `stop_reason_code`：
...
`marvis/app.py` 的中间件只阻止非本机客户端的非安全方法：
...
```text
marvis/api.py                 3246 lines
marvis/pipeline.py            1781 lines
marvis/static/app.js          5907 lines
marvis/static/styles.css      4955 lines
...
- `marvis/api_settings.py` 承载 execution environment 与 LLM settings 路由，`api.py` 通过主 router include，URL 不变。
- `marvis/api_schemas.py` 承载 FastAPI request schema，避免 `api.py` 继续膨胀。
- `marvis/api_task_payloads.py` 承载 task payload、失败阶段、停止状态、报告下载文件名等 UI payload 逻辑，并由 `api.py` 保持兼容导入。
- `marvis/static/js/branding.js` 承载 branding normalize/apply helper。
- `marvis/static/css/welcome.css` 承载欢迎页任务卡/问候样式，`index.html` 用版本占位符链接。
...
1. 后端先拆 router：
   ...
   - `api/memory.py`
   - `api/settings.py`
2. 将 Agent workflow 编排从 `api.py` 移到 `marvis/agent/workflows.py`，API 只做 HTTP 入参、权限和返回 payload。
3. 前端继续拆 ES modules：
...
`marvis/validation/stress_test.py` 在每个特征类别里捕获所有异常：
...
`marvis/static/app.js` 有大量 `innerHTML`、`outerHTML`、`insertAdjacentHTML`。大部分路径已经使用 `escapeHtml()` 或 `renderAgentMarkdown()`，例如任务列表、记忆列表、指标表格、Agent 消息、LLM 设置列表。
...
`marvis/agent_memory/policy.py` 已经拒绝：
...
短期保留手工 cache-bust；中期用应用启动时注入版本号或 git sha；发布包可用 `marvis.__version__` 生成静态资源版本。
...
node --check marvis/static/js/api.js
node --check marvis/static/js/dialogs.js
node --check marvis/static/js/polling.js
node --check marvis/static/js/render-agent.js
node --check marvis/static/js/render-metrics.js
node --check marvis/static/js/state.js
node --check marvis/static/js/ui-utils.js
```
