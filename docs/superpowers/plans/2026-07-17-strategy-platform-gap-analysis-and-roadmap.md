...
**2D/3D Cross 阈值规则搜索、精确物化与报告纵切（已完成，2026-07-27）**：新增自然语言和 Candidate Lab 均可达的 `cross_rule_search` 与 `cross_rule_candidate_build_from_search` Workflow。用户必须显式提供 2 至 12 个字段、维度 2/3、每字段阈值上限、各字段风险方向、坏率/金额约束和最多 5,000 次试验；平台以 5,000 trial、5,000 万 row-evaluation 双预算，按字段组合 round-robin 和固定 tie-break 确定性挖掘 2D/3D `AND` 阈值规则，保留缺失分支、搜索空间、已评估数、截断状态、聚合件数/风险/金额证据及每条约束失败原因，不保存逐行命中矩阵。排序只是 evidence navigation，不产生冠军、推荐、选择、Pool 修改、采纳或部署；后续物化必须由用户逐字提供完整 search id 和 rule id，Tool 在 writer lock 内重新认证搜索、样本、dataset/workspace、target、观测字段和候选重放结果，生成 pointer-bound 不可变 Cross 候选。候选可按 `cross_threshold_rule` 来源进入五类 Pool，Candidate Lab 对搜索和精确候选提供受认证投影；七步报告会冻结并重新认证最新完全兼容的搜索，在 `appendix_cross_rules` 展示最多 50 条开发证据和约束失败明细，同时保持 `backtested/unvalidated`，不会把排序改写为业务选择。人工 Matrix 切点、规则/矩阵代码和专项列写回仍待后续纵切。
