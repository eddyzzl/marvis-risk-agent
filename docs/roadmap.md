...
- Phase 3 的加权自动规则树、Strategy Pool、显式 Voting 和完整 2D Cross Matrix evidence 已完成首批 Agent 纵切：用户可以先用自然语言指定特征及受限参数构建完整候选树，平台确定性生成全量叶节点清单、指标和不可变 full-tree asset；再用完整 tree asset ID 与 leaf ID 物化 pointer-only 叶选择；最后用 selection ID、显式 Pool 类型、默认动作和命中动作把该叶加入 task-scoped `draft / unvalidated` Pool。Pool 对单变量候选、自动树叶和 Voting 统一重放 artifact lineage，使用 revision/hash 并发保护，并支持受治理的删除、动作修改、完整重排和只读编译；同树不同叶可以分别入池，同一叶不能重复。Voting 首纵切要求用户逐字点名当前 Pool 的 2 至 50 个 rule id 和唯一 n，平台只回放这些成员在绑定样本上的 canonical `n_of_k` 条件，输出完整命中数分布、件数及可用金额观测和不可变候选；它不会自动选择“最好规则”，也不会在同一轮入池。用户另行要求入池时必须明确选择保留成员作为后续回退，或由 Voting 原子替代成员；普通末尾追加和无授权全局置顶均被拒绝，编译还会阻止 first-match 遮蔽。2D Cross Matrix 首纵切要求用户明确唯一 X/Y 字段和分箱方法，先生成同一样本的单变量证据，再逐行回放两轴并固化包含空单元格在内的完整 Cartesian matrix、件数/风险及可用金额观测和独立 candidate evidence；本步骤不会自动选格、入池、写回、采纳或部署。所有入池都不会连带采纳、部署或生产变更。交互树、评分卡、Voting 自动组合搜索/人工自定义成员/命中数列写回与代码、Cross 单元格/组合物化与入池、人工切点、2D/3D 自动交叉搜索、逐月稳定性、完整代码交付和其余列写回仍在 V2.x Phase 3 内继续交付，不能据此宣称完整 Candidate Lab 已完成。
