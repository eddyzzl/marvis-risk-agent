...
- 模型验证是稳定兼容工作流之一：继续保留 V1.1 手动模式和 Agent 辅助验证能力，Notebook 契约、PMML 对比、确定性验证指标和 Excel/Word 产出必须保持兼容。欢迎页只保留「模型验证」入口；创建时可添加多个模型。只加一个则与现在单模型验证相同，不生成汇总 Excel；两个及以上走同一套 Agent 工作台并额外出一份汇总 Excel。
...
以下细目按实现先后保留纵切验收记录；其中“仍待”“尚未”等措辞描述的是该条记录当时的边界，当前结论以上述 2026-07-27 收口状态及后续更新为准。
...
- Phase 3 的交互树手工换分裂字段、节点候选搜索和受控自动续建纵切已完成：除剪枝和改阈值外，用户现在可通过自然语言或 Candidate Lab 从来源自动树的认证特征全集中明确选择一个不同字段，并同时给出有限阈值，对当前可见 split node 执行 `replace_split_feature`。平台不会接受“最佳字段”或自动推荐作为写操作，也不允许目标、样本分区或来源树之外的字段；Tool 会重放完整可见拓扑，重新计算目标子树全部节点/前沿的件数、风险、金额、缺失归属和方向诊断，执行最小叶与 exactly-once 守恒后发布不可变 revision，旧树、父 revision 和 sibling 分支保持不变。用户也可对任一当前可见节点选择认证树的全部特征或明确特征子集，在每特征最多 20 个阈值、最多 50 个特征及 2,000 万次行评估的硬预算内生成 task-owned 聚合候选证据；搜索只保留左右节点件数、风险、可用权重、Gini 增益、方向和失败约束，不保存客户明细，不选择胜者，也不修改树。用户确认唯一 `search_id + candidate_id` 后，可再明确最大新增深度、最小 Gini 增益、最多生成节点、每字段阈值数和总行评估预算，让平台按固定目标与 tie-break 对该前沿执行确定性续建；内核不暗选种子、不越过硬预算、不保存逐行样本，并把停止原因、聚合重放和来源指针写入新的不可变 revision。生成节点仍可继续改阈值、换字段、剪枝、选择前沿、入 Pool 和走下游证据链；续建本身不修改 Pool、不采纳、不部署。
- Phase 3/6 的 Candidate Lab 工作台已接通单变量、Cross Matrix、2D/3D Cross 阈值规则搜索及精确构建、自动树、交互树节点全特征/指定特征候选搜索、精确种子受控自动续建、交互树修订/单节点及 OR 分组选择、评分卡 band/cutoff、Voting 自动组合搜索/精确构建、当前 Strategy Pool、五类当前 Pool 的受治理直接应用、Pool 编译/删除/动作修改/完整重排、候选逐月稳定性，以及五类 Pool 的 validation/OOT 独立样本回放验证纵切：策略任务可在宽桌面工作区查看 task-owned、重新验真的投影，并直接启动对应的确定性开发动作。Pool 应用表单只能从当前非空 Pool 投影选择类型并可填写安全 ASCII 输出前缀；Pool revision/hash、样本、数据、workspace 和 requirement 继续由服务端恢复，结果是非 active 派生数据集，不修改 Pool、不采纳或部署。Pool 操作表单只使用完整、非空、未截断的当前认证投影：编译只提交类型，删除和动作修改必须明确选择 Entry，重排始终提交 1 至 200 个 Entry 的完整无重复顺序；操作 settle 后静默刷新投影，均不连带采纳、发布或部署。独立回放表单也改为只显示当前认证非空 Pool；唯一 Pool 自动选择，多个 Pool 必须明确选择，没有兼容 Pool 时失败关闭。Voting 搜索表单只允许从当前 Pool 投影选择已启用且非 Voting 的规则，再提交 K/n、目标、约束与硬预算；Cross 阈值规则搜索也只接受显式字段、维度、方向、约束和硬预算，构建表单只能从受认证投影明确选择 search/rule 指针；两类搜索都不能按名次、最好或冠军自动代选。交互树候选搜索表单只接受当前认证树/revision、可见节点、全特征或明确子集和两个硬预算，结果按钮只回填换字段/改阈值修订表单或续建的精确种子，不自动提交；续建还必须显式给出深度、增益、节点、阈值和行评估上限，使用固定决策策略。单节点表单只允许从当前 revision 的认证 frontier 拣选 singleton 指针，OR 分组表单只允许明确多选 2 至 50 个同 revision frontier 指针，点击或多选只负责创建 pointer-only selection，不会在同一动作中入池。独立回放表单只接受 Pool 类型和 validation/OOT 分区，Pool revision/hash、样本、数据及模型分数 requirement 全部由服务端恢复；它输出 absolute replay 证据，不计算或声称 PSI、稳定性或漂移，也不修改 Pool、采纳或部署。手工表单与自然语言 Agent 共用同一 typed request compiler、PlanValidator、Workflow 和确定性 Tool；已有候选只能从当前投影选择 candidate/feature/method/bin、交互树 search/revision/frontier、band/cutoff、Voting search/combo、Cross search/rule 或 Pool entry，artifact/hash、Pool revision/hash、样本和数据绑定继续由服务端恢复。投影查询在数据库层按类型计数并只读取最新窗口，使用总字节预算、来源 artifact canonical/provenance 重验和同源缓存；前端按任务 single-flight 刷新，切换任务会取消旧请求，不进入每秒轮询。完整 Evidence Drawer 也已接通：只批量读取当前任务已进入受认证投影的产物，展示 dataset lineage、artifact/tool/契约版本/生成器版本、内容 hash、来源绑定 hash、明确的原生输入 hash 或标注为 provenance 派生的绑定摘要、红旗及最近 Agent 回复中的记忆审计指针；不读取原始客户行、不暴露路径或完整 provenance，也不把自由文本当业务事实。该纵切仍不是完整 Workbench：其余七区状态持久化、长任务控制和完整 browser/API 旅程仍须在 V2.x 内完成。
- Candidate Lab 的首个 task-scoped 视图状态纵切已完成：同一台本地 MARVIS 上切换任务或重启页面后，会按任务恢复已打开的操作区、当前受认证下拉/多选节点、cutoff/cell/Pool 选择、数值预算和人工切点；恢复时只接受当前选项集合，已失效的节点或 cell 不会重新注入，最终提交仍按最新服务端投影重验。浏览器状态采用 64 KiB 硬上限，只保存门禁选择和数值控制，不保存项目说明、选择理由、报告字段或其他自由文本。字段别名、语言和跨设备/多用户同步仍属于后续状态纵切。
...
- **模型验证**：保持 V1.1 既有手动/Agent 验证能力可用，并可通过 `v1_compat` 作为 Workflow 里的稳定工具包调用。创建时可添加多个模型：N=1 只出该模型 Word/Excel；N≥2 同一套工作台，并额外生成不重算指标的汇总 Excel。多模型编排规格见 [模型验证多模型规格](plans/specs/v2-validation-batch-spec.md)。
