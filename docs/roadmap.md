...
- Phase 3 的加权自动规则树和 Strategy Pool 已完成首条 Agent 三轮纵切：用户可以先用自然语言指定特征及受限参数构建完整候选树，平台确定性生成全量叶节点清单、指标和不可变 full-tree asset；再用完整 tree asset ID 与 leaf ID 物化 pointer-only 叶选择；最后用 selection ID、显式 Pool 类型、默认动作和命中动作把该叶加入 task-scoped `draft / unvalidated` Pool。Pool 对单变量候选和自动树叶统一重放 artifact lineage，使用 revision/hash 并发保护，并支持受治理的删除、动作修改、完整重排和只读编译；同树不同叶可以分别入池，同一叶不能重复。树结果可按用户明确授权写入不可变派生数据集，但入池不会连带采纳、部署或生产变更。交互树、评分卡、Voting、Cross、逐月稳定性、完整代码交付和其余列写回仍在 V2.x Phase 3 内继续交付，不能据此宣称完整 Candidate Lab 已完成。
