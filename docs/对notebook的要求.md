...
本文件只描述 MARVIS V2.1.6 当前内置的模型验证工作流，不代表平台只能做模型验证。该工作流会执行开发人员提交的 Jupyter Notebook，并使用 Notebook 运行后留在内存里的模型对象进行打分，再与开发人员提交目录中的 PMML 模型打分结果进行一致性比较。
...
- 平台会在统一的比较函数中按该配置判断代码模型分和 PMML 模型分是否一致；默认口径下，
  6 位小数四舍五入一致率达到 99% 且最大绝对差不超过平台容忍阈值时，分数一致性判定为通过。
