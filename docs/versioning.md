...
- **V1.0.x**：当前稳定模型验证工作流。
- **V1.1**：计划中的 Agent Memory Foundation，仍属于 V1 产品线；包含用户偏好、字段口径、验证坑点、任务经验、模型经验和 skill 经验预留。
...
- `V1.0.2`：当前 V1 patch 版本。
- `V1.1.0`：加入 Agent Memory Foundation 的 V1 minor 版本，记忆只辅助解释、建议、历史对比和审计，不改变确定性验证结果。
...
1. 要求工作区干净。
2. 从最新 `V<MAJOR>.<MINOR>.<PATCH>` tag 计算下一版本，或使用 `--version` 指定值。
...
1. **保持 V1 稳定**
   ...
   V1.0.x 和 V1.1 用于演示稳定性、bugfix、兼容性、Agent 记忆和低风险增强。V1.1 可以预留 skill 经验 schema，但不要把未稳定的 V2 plugin/runtime 架构混入 V1 稳定线。
...
```text
/path/to/riskmodel_checker-v1   V1 稳定线和 V1.1 memory 线
/path/to/riskmodel_checker-v2   V2 plugin/tool runtime 线
```

要求 Agent 工作时明确路径：

```text
只在 /path/to/riskmodel_checker-v1 开发 V1.1 memory。
只在 /path/to/riskmodel_checker-v2 开发 V2 plugin/tool runtime。
```

不要在一个 Codex/Claude 线程中来回切换 major worktree，除非任务本身就是 forward-port 或冲突处理。
...
如果只是文档修订，通常只需要 `git diff --check`。最终说明中写明未运行代码测试的原因。
