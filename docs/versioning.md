...
- **V2.x**：当前主线及全部已确定产品范围；除既有多工作流外，还承载完整 Strategy/Portfolio Workbench、组织与生产治理、调度告警、实时评分、决策引擎对接和更强执行隔离。具体产品范围以 `docs/roadmap.md` 为准。
- **V1.1.x**：模型验证兼容基础能力，已包含 Agent Memory Foundation；记忆只辅助解释、建议、历史对比和审计，不改变确定性验证结果。
- **V1.0.x**：上一条稳定模型验证线。
- **V3/V4**：仅保留为未来可能使用的 major 标签，当前不分配产品能力，也不得作为延后 V2 已确定范围的蓄水池。
...
- `V2.2.0-alpha.1`：V2.x 新一批用户可见能力的预发布版本。
...
发布前检查必须运行不带 `--affected`/`--fast` 的完整 `scripts/check`。可选加 `--audit` 跑 `pip-audit` 依赖 CVE 扫描（未安装时打印跳过原因，不会失败）。
