...
> **快速统计**：原始基线为 179 项；2026-07-17 新增范围以 10 个 FSP 里程碑单独计数，避免把大型里程碑与原子缺陷项直接相加。

---

## 0A. V2 全功能扩展里程碑（2026-07-17）
...
| 实时评分 API + 决策引擎对接 | **V2 重新纳入（2026-07-17）** | 进入策略平台计划 Phase 8；包含离线/批量/实时等价、逐环境 shadow、promotion 和 rollback。 |
| 监控调度器 + 推送告警 | **V2 重新纳入（2026-07-17）** | 进入策略平台计划 Phase 7/8；包含 scheduler、日历、幂等、重试、通知和生产 SLO。 |
| 征信报文解析（人行二代/三方报文） | **推迟** | 工程量最大，需另立探索 spec 后评估。 |
| maker-checker 多角色审批 | **V2 重新纳入（2026-07-17）** | 进入策略平台计划 Phase 8；真实身份、RBAC、职责分离和生产授权一并设计。 |
...
| ⬜ | LT-9 | **2026-07-17 重开**：V2 纳入多用户与生产治理，原“subprocess 为 V2 终态”失效；按策略平台计划 Phase 8 补更强隔离和多租户边界 | 新范围决定 |
...
| ⬜ | LT-18 | **2026-07-17 迁回 V2**：多用户、生产/远程部署、决策引擎、服务化、调度监控和相关资源治理不再进入 V3+ 蓄水池 | 新范围决定 |
