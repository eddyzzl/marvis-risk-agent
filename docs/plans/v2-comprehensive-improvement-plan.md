...
2. File and DB writes still need recovery semantics, but the main artifact paths now use staged promotion.
   - Current state: join execution output, route-level raw dataset uploads, ordinary dataset upload parquet outputs, data_ops clean/dedup derived outputs, modeling derived parquet/model/meta/PMML/calibration outputs, Excel multi-sheet upload parquet outputs, report scored parquet output, final xlsx reports, plugin install/promote directories, validation handoff materials, V1 metrics outputs/model meta, and V1 validation report docx/images now use staged file or directory promotion.
   - Fix:
