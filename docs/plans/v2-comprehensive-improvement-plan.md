...
3. G4 model selection is automatic.
   ...
   - Fix:
     ...
     - Remaining: broaden native/export and policy-report language fixtures.
...
7. Hard-coded metric gates conflict with "no fixed metric target".
   ...
   - Fix:
     ...
     - Remaining: broaden policy report language so pass/warn/fail thresholds are always visibly configurable rather than implied by one universal metric target.
...
- Done: add policy-driven thresholds for modeling acceptance through `selection_policy.metric_thresholds` and common shortcuts.
