...
2. `db.py` is still repository-heavy, but schema/connection setup has started moving out.
   - Fix order:
     ...
     - `marvis/repositories/tasks.py`, `plans.py`, `datasets.py`, `plugins.py`, `audit.py`, `modeling.py`.
...
- Partial: split `db.py` schema/connection setup into `marvis/db_schema.py`; `StrategyRepository` and `DraftRepository` now live in `marvis/repositories/strategy.py` and `marvis/repositories/drafts.py`; remaining repositories still need module extraction.
