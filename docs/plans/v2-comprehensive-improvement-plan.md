...
| 3 | PlanDriver decomposition | Partial | Tool output renderers moved to `marvis/agent/renderers.py`; structured gate payload builders moved to `marvis/agent/gate_payloads.py`; gate dependency rendering moved to `marvis/agent/gate_adapters.py`; gate response/control validation moved to `marvis/agent/gate_response_adapter.py`; basic adjust parameter specs moved to `marvis/agent/adjust_specs.py`. `PlanDriver` still owns adjust/replan execution routing and some step-specific reset/apply behavior. | Continue moving adjust/replan execution into schema-driven adapters. |
