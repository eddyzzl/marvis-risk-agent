...
1. `app.js` decomposition is unfinished.
   ...
   - Fix order:
     ...
     - `DriverConversationView`: manual analysis rendering, dedup/C1 controls, agent transcript DOM mounting.
...
- Started `DriverConversationView`: timeline stages, frozen snapshot re-anchoring, message bucketing, report-message filtering, and advance-intent detection live in `static/js/agent-conversation-view.js`; DOM mounting and C1/dedup gate submit controllers remain in `app.js`.
