...
2. `db.py` is still repository-heavy, but schema/connection setup has started moving out.
   - Fix order:
     ...
     - `marvis/repositories/tasks.py`, `plans.py`, `audit.py`, `modeling.py`.
...
- Partial: split `db.py` schema/connection setup into `marvis/db_schema.py`; `StrategyRepository`, `DraftRepository`, `PluginRepository`, and `DatasetRepository` now live in `marvis/repositories/strategy.py`, `marvis/repositories/drafts.py`, `marvis/repositories/plugins.py`, and `marvis/repositories/datasets.py`; remaining repositories still need module extraction.
