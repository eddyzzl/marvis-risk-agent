...
3. Finish the remaining modeling productization with manual business smoke plus native/export edge-case fixtures. JSON and Markdown model cards, approval packages, monitoring policies, prior Champion comparison, PMML/native delivery metadata, PMML-backed challenger/backtest packages, and stricter scorecard/monotonicity policy fixtures now exist and have browser or unit-level coverage.
...
1. G2 algorithm/task selection now has a typed backend step and editable UI controls.
   ...
   - Remaining problem: manual business smoke plus native/export and broader domain-policy fixtures still need coverage.
   - Remaining fix:
...
2. Modeling needs a dedicated setup and analysis surface.
   - Fix:
     ...
     - Remaining: manual business smoke plus native/export and broader domain-policy edge-case coverage.
