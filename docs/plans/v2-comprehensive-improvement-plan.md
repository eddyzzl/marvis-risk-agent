...
5. Continue deeper `api.py`, `db.py`, and `app.js` decomposition after the first stable splits. Low-coupling GET routes have started moving out of `api.py`: `/api/branding` now lives in `marvis/routers/branding.py`, and `GET /api/tasks` now lives in `marvis/routers/tasks.py`; task creation/detail/stage routes still need a domain router split.
...
- `api.py`, `db.py`, and `app.js` remain large after the first splits. `turn_handlers.py`, `api_data_payloads.py`, `agent_memory/api_support.py`, `routers/agent_memory.py`, `routers/branding.py`, `routers/tasks.py` for task listing, `db_schema.py`, `theme.js`, `task-types.js`, `create-task-dialog.js`, renderers, gate payloads, and adjust specs are good first cuts, not the final architecture.
...
1. `api.py` remains a bottleneck.
   - Current state: driver turn orchestration for data_join, feature_analysis, modeling, strategy, and vintage has moved to `marvis/agent/turn_handlers.py`; Agent Memory HTTP routes are now in `marvis/routers/agent_memory.py`; memory payload/context/audit support is in `marvis/agent_memory/api_support.py`; `/api/branding` and `GET /api/tasks` are now mounted from dedicated routers. `api.py` still owns task creation/detail routes, validation/data/stage job routes, and validation-agent compatibility wrappers.
   - Fix order:
     ...
     - Done: `marvis/routers/tasks.py`: task list endpoint and pagination headers.
...
- Done: split Agent Memory routes, branding, and task-list GET routes.
- Continue splitting data/validation/task/agent routers.
