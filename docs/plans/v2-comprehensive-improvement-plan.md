...
- Current focused verification from this implementation pass:
  ...
  - `CONDA_NO_PLUGINS=true conda run -n py_313 pytest tests/test_modeling_api.py -q`: `9 passed` after the real business-material smoke was added to the conversational modeling API suite.
...
3. Finish the remaining modeling productization with the native/export edge-case fixtures and broader domain-policy coverage. Real business-material smoke now exists for conversational modeling: source-dir business columns and feature dictionary are auto-detected, preserved through the split frame, and verified through report/delivery outputs. JSON and Markdown model cards, approval packages, monitoring policies, prior Champion comparison, PMML/native delivery metadata, PMML-backed challenger/backtest packages, stricter scorecard/monotonicity policy fixtures, native LightGBM/XGBoost Booster skip coverage, and the current Playwright Chromium smoke now exist.
...
3. G4 model selection is automatic.
   ...
   - Fix:
     ...
     - Done: run real business-material smoke through the conversational modeling API, including business columns, feature dictionary, report sections, and delivery artifacts.
...
4. G5 post-training closure is not a workflow.
   ...
   - Fix:
     ...
     - Done: cover native LightGBM/XGBoost Booster G5 delivery close so unsupported PMML/handoff actions skip with explicit limitations and no validation task side effects.
     ...
     - Remaining:
       - real business-material smoke is now covered by `test_modeling_business_materials_flow_into_report_and_delivery`.
