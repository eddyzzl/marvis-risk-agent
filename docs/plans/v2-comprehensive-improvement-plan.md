...
- Started `DriverConversationView`: timeline stages, frozen snapshot re-anchoring, message bucketing, report-message filtering, and advance-intent detection live in `static/js/agent-conversation-view.js`; DOM mounting and generic driver confirmation remain in `app.js`.
