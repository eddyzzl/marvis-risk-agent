...
1. `app.js` decomposition is unfinished.
   ...
   - Fix order:
     ...
     - Done: `TaskWorkspaceView` owns current-task title/subtitle rendering, empty-workspace class toggling, greeting DOM writes, and hero-layout sync scheduling.
...
- Started `TaskWorkspace`: selected-task storage, result-scroll storage, empty-workspace greeting rules, greeting DOM writes, and current-task shell DOM rendering live in `static/js/task-workspace-state.js` / `static/js/task-workspace-view.js`; active task orchestration and right-rail coordination remain in `app.js`.
