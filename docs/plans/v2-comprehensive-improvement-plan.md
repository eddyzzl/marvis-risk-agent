...
2. File and DB writes still need recovery semantics, but the main artifact paths now use staged promotion.
   ...
   - Fix:
     ...
     - Done for runtime sidecar writes: shared `write_text_atomic` / `write_json_atomic` helpers now cover notebook progress snapshots, worker-returned notebook step events, scan preview step JSON, metrics cancel markers, memory policy settings, and notebook cancel/resource-limit logs.
