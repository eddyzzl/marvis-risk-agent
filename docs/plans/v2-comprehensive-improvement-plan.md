...
2. `db.py` is still repository-heavy, but schema/connection setup has started moving out.
   - Fix order:
     ...
     - `marvis/repositories/tasks.py`, `plans.py`, `datasets.py`, `audit.py`, `modeling.py`.
...
- Partial: split `db.py` schema/connection setup into `marvis/db_schema.py`; `StrategyRepository`, `DraftRepository`, and `PluginRepository` now live in `marvis/repositories/strategy.py`, `marvis/repositories/drafts.py`, and `marvis/repositories/plugins.py`; remaining repositories still need module extraction.
