...
2. Expensive jobs are mixed across direct request handlers and background tasks.
   ...
   - Current update: `/api/joins/{join_plan_id}/execute` still supports synchronous execution when explicitly requested, and the V2 frontend join wrapper now defaults to background execution via `async_execute: true`. The async path creates a task job with `kind="join"`, rejects existing active jobs, records succeeded/failed job state from a fresh data runtime, and `join_review.js` polls the accepted `job_id` path by refreshing the join plan until execution completes.
   - Remaining: expose job failure details through a first-class task/job status API and wire the main app workspace to the V2 join component before treating join execution as fully productized background work.
