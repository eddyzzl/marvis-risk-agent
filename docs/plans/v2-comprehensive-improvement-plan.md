...
- `api.py`, `db.py`, and `app.js` remain large after the first splits. `turn_handlers.py`, `api_data_payloads.py`, `agent_memory/api_support.py`, `routers/agent_memory.py`, `db_schema.py`, `theme.js`, `task-types.js`, renderers, gate payloads, and adjust specs are good first cuts, not the final architecture.
...
1. `app.js` decomposition is unfinished.
   - Current state: V2 modules exist, theme handling has moved to `static/js/theme.js`, and create-dialog task type definitions/display ordering now live in `static/js/task-types.js`, but task dialog behavior, plan rail, conversation rendering, and task creation still live in one global controller.
   - Fix order:
     - Partial: `CreateTaskDialog`: task type definitions and display order are extracted; run mode, algorithm family mutual exclusion, material source, tier picker, and payload assembly remain.
...
Tasks:
- Partially extracted `CreateTaskDialog`: task type definitions and display order now live in `static/js/task-types.js`; controller behavior remains.
...
- `api.py`, `db.py`, and `app.js` are still large, but the first stable splits have landed (`turn_handlers`, `api_data_payloads`, `agent_memory/api_support`, `routers/agent_memory`, `db_schema`, `theme.js`, `task-types.js`); remaining risk is deeper router/repository/workspace-controller extraction.
