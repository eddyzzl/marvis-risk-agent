...
> 目标状态（用户原话）：欢迎页 6 个入口都能直接进对应任务；agent 模式下走 V2 DAG，通过对话让 LLM 编排+执行并在右栏展示流程；顶栏只展示报错/整体状态；拆出的每个步骤可在中间对话流可视化确认并按用户输入调整；类似模型验证 agent 的"大步骤套小步骤"。当前实现口径：数据拼接 / 特征分析 / 模型开发 / 策略开发 / Vintage 风险分析均接入 PlanDriver；模型验证保留原验证流程。
...
新建的件（全计划仅这些是"新"）：
- **3 个 phase 模块 + 组合模板**（JOIN/FEATURE/MODELING），并补策略 / Vintage 的独立 PlanDriver 模板。
...
## 五、范围与不动项
- **做**：数据拼接 / 特征分析 / 模型开发 / 策略开发 / Vintage 风险分析（均走 PlanDriver；策略与 Vintage 先交付可执行的最小闭环）。
- **不改**：模型验证（保留其现有 手动/agent 与写死步骤、旧 `#progressPanel`；按 task_type 与新 UI 共存）。
- 6 入口直达对应任务为目标；欢迎页不得展示不可执行入口。

### 周边模块定位（2026-06-25 澄清）
- **记忆**：现仅验证管道自动沉淀（`pipeline.py`，受 auto_distill 门控）+ 聊天偏好捕获。**PlanDriver 新流程本轮不接记忆写入**（后续增强：接则记各门复发决定/偏好做默认建议，须门控，注意"两条写记忆路径"坑）。
...
6. 🚧 [策略开发与风险分析完善计划](v2-strategy-risk-analysis-plan.md)（第四阶段模块 STRATEGY + ANALYSIS 模板族；打分/方向底座 + cutoff/规则/额度定价/采纳门 + 组合分析/监控闭环/即席分析；6 项已拍板，函数级 spec 后续逐份出）。

> **待办跟踪**：全部剩余工作（审查发现、极致审计、策略批次、PR 流程、长线项）的唯一权威清单是 [v2-master-backlog.md](v2-master-backlog.md)（2026-07-02 起），进度只在该文档更新。
