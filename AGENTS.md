...
- **V1.1.x**：当前稳定模型验证工作流，已包含 Agent Memory Foundation。
- **V1.0.x**：上一条稳定模型验证线。
...
- **Workflow**：Agent 生成、平台内置、或用户可编写的 Tool/Hook 工作序列（模板）。
- **Skill**：SOP / Playbook / 方法论型知识，**落地为「用户可编写的 Workflow 模板」**（Phase 2 Part C2）——声明式、只编排已信任工具、实例化后过 `PlanValidator`，不另立 runtime。历史文档可能把它作为 plugin/tool runtime 的旧称。

skill 不是独立 runtime 而是 Workflow 模板的一种来源（用户编写 vs 随包内置）。不要在新文档里把 “skill runtime” 当作独立的第 4 套执行抽象；执行面统一是 Plugin / Tool / Hook / Workflow。
...
agent_memory/   V1.1 已落地能力边界；存储、检索、压缩和审计记忆，不直接改变任务指标
...
- V1.1 已覆盖用户偏好、字段口径、验证坑点、任务经验和模型经验；skill 经验（skill = 用户可编写 Workflow 模板）只预留 schema 和接口，V1.1 不落地 V2 能力。
