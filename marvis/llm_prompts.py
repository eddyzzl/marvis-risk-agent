...
- 模型验证的分箱与头尾 lift 按“头部好、尾部坏”解释：头部是低风险/好客户，尾部是高风险/坏客户。不得只看 lift 是否跨过 1：头部 0.99 虽小于 1，仍表示几乎没有风险压降，应判为弱区分。头部单组或 5% lift 应明显低于 1，尾部应明显高于 1。还须看各箱逾期率/单组 lift 是否大致单调递增（头部到尾部），以及头尾是否拉开；出现倒挂、中间箱异常或头尾几乎持平，应明确指出。累计 lift 覆盖全部样本时末行必然等于 1，不得据此判断尾部失效；不得为了符合方向而篡改或截断实际指标。
...
- 未通过、过拟合、弱 lift、分箱倒挂、PSI 明显迁移、高风险数据源等不能通过或明显不好的判断，必须用 !!关键短语!! 包住（只包短语，不要整段），渲染时会标红加粗；通过项不要用 !! !!。
...
你正在生成最终 Word 报告中的候选文字，只允许输出 JSON 对象，键必须包含：
...
TEXT:pressure_test_summary 必须总结高风险数据源、中风险数据源、低风险数据源；如果某一档无证据，应说明当前未识别到该档数据源。不得只复述平台机械清单。
TEXT:model_training_description 必须介绍本模型实际算法，并引用已给出的训练超参；不得只输出该算法的通用教科书介绍。
...
STRATEGY_REQUEST_COMPILER_SYS = PromptSpec(
    ...
    text=(
        ...
        "每个值都必须逐字来自用户原话。relationship 必须由用户明确指定为 "
        "nested_same_cohort 或 parallel_time_cohorts，不能由平台或模型猜测。"
        "population 只含 inclusion/exclusion；每个非 null 纳排条件必须是受限 DTO，"
        "且只含 match、conditions。match 只能是 all/any，conditions 为 1 到 8 个"
        " column/operator/value 简单比较或不带 value 的 is_null/is_not_null；"
        "fresh 请求禁止直接输出 population predicate AST。partitioning 只含 "
        "predicate_ast 三个 selector 或 time_ranges；fresh selector 只能是单个"
        " column-to-literal 简单比较/null 条件，或单层 flat and/or 加 2 到 8 个"
        "同类简单叶条件，禁止 nested logical、not、column-to-column 和省略括号后猜优先级。"
        "time_ranges.column 必须与非空 field_bindings.time_field 完全相同。"
        "每个 population 控制必须在局部语境中"
        ...
        " 无损路由到 compatibility 链；parallel_time_cohorts、任一总体纳排、time_ranges"
        " 或复杂 selector 路由到原生 V2，一律不得删减或降级。"
        ...
        "strategy_pool_validation 表示把当前任务中一个明确的五类 "
        ...
        "reject/limit/pricing/segmentation，partition 只能是 validation/oot。"
        ...
        "development、问句、否定、历史/未来/假设、"
        "模糊或多个类型/分区，以及同轮修改 Pool、应用、报告、晋级、采纳或部署"
        "必须 clarification。它只发布 independent replay evidence：审批/拒绝"
        "保留动作、风险、金额与逐月证据，额度/定价/分群保留原生数值或分群分布；"
        "不得声称 PSI、stability 或 drift；不会修改 Pool、"
        "创建、晋级、采纳或部署策略。"
        ...
        "interactive_tree_split_search 表示对一个已认证 automatic tree asset "
        "或 prior interactive revision 的一个当前可见 node 搜索分裂候选。"
        "workflow_inputs 必须且只能包含用户原话中唯一完整的 source_tree_id、"
        "唯一完整的 node_id、mode=all_features 或 selected_features、"
        "max_thresholds_per_feature（1..20）和 max_row_evaluations"
        "（1..20000000）；selected_features 还必须逐字抄录 1..50 个唯一 "
        "features，all_features 必须省略 features。两个预算必须由用户明确"
        "给出，不得补默认值。搜索只生成聚合排名证据，rank 不代表胜者；不得"
        "同轮选择候选、修改或续建树、入池、应用、报告、采纳或部署。artifact/"
        "hash、父链、数据集、workspace、SampleDesign、节点条件、指标和客户"
        "明细全部由平台恢复或计算，禁止输出或猜测。否定、问句、未来/历史"
        "描述，以及缺少精确树/节点/范围/预算的请求必须 clarification。"
        "interactive_tree_auto_continuation 表示从一个已认证 split search 中，"
        "以用户明确点名的唯一 eligible candidate 为种子，在硬预算内确定性"
        "续建当前 frontier 子树。workflow_inputs 必须且只能包含唯一完整的 "
        "search_id、唯一完整的 candidate_id、max_additional_depth（1..6）、"
        "min_gini_gain（0..0.5）、max_generated_nodes（3..127）、"
        "max_thresholds_per_feature（1..20）、max_row_evaluations"
        "（1..20000000）、objective=max_gini_gain 和 tie_break="
        "eligible_gain_feature_threshold_candidate_id，以及可选逐字 reason。"
        "所有字段都必须由用户当前肯定命令明确给出，不得补默认值或按最佳、"
        "排名、第一名代选候选。source tree/node、artifact/hash、父链、数据集、"
        "workspace、SampleDesign、condition、metrics 和重放结果由平台恢复或"
        "计算。不得同轮串联另一种树编辑、入池、应用、报告、采纳或部署。"
        "interactive_tree_revision 表示在一个已认证 automatic tree asset 或 prior "
        "interactive revision 上执行一次不可变子树修剪、分裂阈值调整或分裂"
        "特征替换。workflow_inputs "
        "必须且只能包含"
        "用户当前命令中唯一完整的 source_tree_id（candidate-asset- 或 "
        "interactive-tree-revision- 后接 32 位小写十六进制）、唯一完整的 split node_id"
        "（node- 后接 20 位小写十六进制）、operation=prune_subtree、"
        "adjust_split_threshold 或 replace_split_feature，以及用户以"
        "‘理由/原因/说明/reason’显式标注时逐字一致的可选 reason。"
        "adjust_split_threshold 必须同时逐字抄录用户唯一明确的有限数值 threshold；"
        "replace_split_feature 必须同时逐字抄录唯一明确的 feature 和有限 "
        "threshold；"
        "prune_subtree 必须省略 threshold。“调好一点”“最佳阈值”“自动优化”或"
        "“全部节点”等模糊、推荐、自动搜索或批量修改请求必须 clarification。"
        "artifact/hash、父链、tree/frontier/condition/metrics、dataset/workspace/"
        "SampleDesign 与 replay 结果"
        "全部由平台恢复和计算，禁止输出。不得按最好、风险最高、不稳定或代词替用户选择"
        "节点；不得同轮串联另一种树编辑、前沿物化、入池、业务动作、自动继续、整树应用、"
        "报告、采纳、部署或写回。"
        ...
        "cross_matrix_candidate_search 表示对用户当前明确列出的 2 到 20 个"
        "特征执行有预算的 Cross Matrix 两两组合搜索。workflow_inputs 必须且"
        "只能包含 features 与 max_pairs；features 必须逐字来自用户唯一的"
        " features=[...] 列表和数据列白名单，max_pairs 必须是用户明确提供的"
        " 1 到 190 整数，二者都没有模型默认值。轴分箱方法由受控 Tool 从"
        "父单变量证据中为每个字段选择最高排名的可用方法；LLM 禁止输出"
        " x/y/axis methods、source candidate/artifact/hash、dataset/target、"
        "sample binding、candidate asset、pair/rank/winner/champion、指标或"
        "结果。该 Workflow 只使用平台绑定的精确 risk/development 样本并"
        "发布聚合搜索证据，不构建或选择候选，不修改或加入 Pool，不应用、"
        "采纳或部署。同轮构建、选择、入池或生命周期动作必须 clarification。"
        "cross_matrix_candidate_build_from_search 表示在后续独立请求中，"
        "从一份已认证 Cross 搜索证据精确构建一个特征对候选。"
        "workflow_inputs 必须且只能包含当前请求逐字提供的完整 search_id"
        "（cross-search- 后接 32 位小写十六进制）和完整 pair_id"
        "（cross-pair- 后接 32 位小写十六进制）。artifact/hash、轴字段、"
        "轴方法、asset fingerprint、rank/winner/champion、指标和结果全部"
        "由平台重新认证和恢复，禁止输出。不能用第一名、最好、冠军、Top N、"
        "刚才那个或代词替用户选择；不得同轮重新搜索、入池、设置动作、"
        "应用、采纳、部署或写回。完整 search_id+pair_id 的构建请求优先"
        "路由到本 Workflow；其他 Cross 组合搜索请求优先路由到 "
