...
_INPUT_FIELDS = frozenset(
    {"revision_id", "source_node_ids", "selection_reason"}
)
...
class VerifiedInteractiveTreeFrontierGroupSelection:
    """One live group selection plus its recursively authenticated revision."""
    ...
    def artifact_binding(self) -> dict[str, Any]:
        """Project exact live TaskArtifact facts for deterministic replay."""
...
def run_materialize_interactive_tree_frontier_group_selection(
    inputs: object,
    ctx,
    runtime,
) -> dict[str, Any]:
    """Persist one pointer-only explicit OR group from a live revision."""
    ...
    revision = load_verified_interactive_tree_revision(
        ...
        max_total_bytes=MAX_INTERACTIVE_TREE_FRONTIER_GROUP_REVISION_BYTES,
    )
    ...
    if len(canonical) > MAX_INTERACTIVE_TREE_FRONTIER_GROUP_SELECTION_BYTES:
        raise StrategyError(
            "interactive-tree frontier group selection exceeds the JSON byte budget"
        )
    content_hash = hashlib.sha256(canonical).hexdigest()
    provenance = interactive_tree_frontier_group_selection_provenance(
        selection
    )
    artifact, artifact_id, final_path = _persist_selection(
        runtime,
        task_id=task_id,
        request=request,
        revision=revision,
    ...
    return {
        ...
        "rule_id": fragment_ref["rule_id"],
        "effect_id": fragment_ref["effect_id"],
        "artifacts": [artifact],
    }


def canonical_interactive_tree_frontier_group_selection_path(
    tasks_dir: Path | str,
    *,
    task_id: str,
    selection_id: str,
) -> Path:
    """Return the sole task-owned path for one immutable group selection."""
    ...
    if _SELECTION_ID_RE.fullmatch(normalized_selection) is None:
        raise StrategyError(
            "interactive-tree frontier group selection_id has an invalid format"
        )
    return (
...
def interactive_tree_frontier_group_selection_provenance(
    selection_payload: Mapping[str, Any],
) -> dict[str, Any]:
    """Derive exact registry metadata from a canonical group pointer."""
    ...
    try:
        ...
        raise StrategyError(
            "interactive-tree frontier group selection failed validation"
        ) from exc
    if set(provenance) != _PROVENANCE_FIELDS:
        raise StrategyError(
            "interactive-tree frontier group selection provenance fields drifted"
        )
    return provenance


def load_verified_interactive_tree_frontier_group_selection_artifact(
    runtime,
    *,
    task_id: str,
    artifact_id: str,
    ...
    """Load a group and recursively re-authenticate its revision chain."""
...
def load_verified_interactive_tree_frontier_group_selection_artifact_on_connection(
    conn,
    *,
    runtime,
    task_id: str,
    ...
    normalized_asset_id = _required_text(
        expected_asset_id,
        "expected_asset_id",
    )
    ...
    if row is None:
        raise StrategyError(
            "interactive-tree frontier group selection artifact was not found"
        )
    record = {field: row[field] for field in _TASK_ARTIFACT_ROW_FIELDS}
    _require_exact_fields(
        record,
        _TASK_ARTIFACT_ROW_FIELDS,
        "interactive-tree frontier group selection registry row",
    )
    fixed_record = {
    ...
    for field, expected in fixed_record.items():
        ...
        if not matches:
            raise StrategyError(
                "interactive-tree frontier group selection registry "
                f"{field} changed"
            )
    ...
    expected_path = canonical_interactive_tree_frontier_group_selection_path(
    ...
    if path != expected_path:
        raise StrategyError(
            "interactive-tree frontier group selection path is not canonical"
        )
    ...
    if not hmac.compare_digest(normalized_artifact, expected_artifact_id):
        raise StrategyError(
            "interactive-tree frontier group selection stable identity changed"
        )
    canonical_bytes = revision_tools._read_verified_regular_file(
        ...
        max_bytes=MAX_INTERACTIVE_TREE_FRONTIER_GROUP_SELECTION_BYTES,
    )
    ...
    if not hmac.compare_digest(canonical_bytes, expected_canonical):
        raise StrategyError(
            "interactive-tree frontier group selection is not canonical JSON"
        )
    expected_provenance = (
        interactive_tree_frontier_group_selection_provenance(selection)
    )
    if not hmac.compare_digest(
        ...
        raise StrategyError(
            "interactive-tree frontier group selection provenance changed"
        )
    ...
    if rebuilt != selection:
        raise StrategyError(
            "interactive-tree frontier group selection changed from its revision"
        )
    return VerifiedInteractiveTreeFrontierGroupSelection(
...
def _build_selection(
    ...
    *,
    ...
    ancestry = revision.ancestor_revisions
    try:
        return build_interactive_tree_frontier_group_selection(
            revision.revision,
            revision.automatic_source.asset,
            revision_artifact_binding=revision.builder_binding(),
        ...
        raise StrategyError(
            "interactive-tree frontier group selection is invalid: "
            + str(exc)
        ) from exc
...
def _persist_selection(
    ...
    *,
    ...
    final_path = canonical_interactive_tree_frontier_group_selection_path(
        runtime.settings.tasks_dir,
        task_id=task_id,
        selection_id=selection["selection_id"],
    )
    if final_path.parent != out_dir:
        raise StrategyError(
            "interactive-tree frontier group selection output path drifted"
        )
        ...
        with runtime.task_artifacts.transaction() as conn:
            ...
            try:
                ...
                if locked_revision != revision:
                    raise StrategyError(
                        "interactive-tree revision changed before group "
                ...
                if locked_selection != selection or not hmac.compare_digest(
                    ...
                    raise StrategyError(
                        "interactive-tree frontier group selection changed "
                        "before registration"
                    )
                if not hmac.compare_digest(
    ...
    *,
    ...
    if row is None:
        if final_path.exists() or final_path.is_symlink():
            raise StrategyError(
                "interactive-tree frontier group selection path exists "
                "without a registry row"
            )
        return
    _verify_registered_selection(
        {field: row[field] for field in _TASK_ARTIFACT_ROW_FIELDS},
        task_id=task_id,
        final_path=final_path,
        content_hash=content_hash,
        provenance=provenance,
...
def _verify_selection_file(
    ...
    *,
    ...
    if not hmac.compare_digest(persisted, expected_content):
        raise StrategyError(
            "interactive-tree frontier group selection artifact bytes changed"
        )
    ...
    *,
    ...
    try:
        ...
        if out_dir.is_symlink():
            raise StrategyError(
                "interactive-tree frontier group selection directory must "
                "not be a symlink"
            )
        out_dir.mkdir(exist_ok=True)
        if out_dir.resolve(strict=True).parent != task_dir.resolve(strict=True):
            raise StrategyError(
                "interactive-tree frontier group selection directory escaped "
                "task storage"
            )
    except OSError as exc:
        raise StrategyError(
            "interactive-tree frontier group selection directory is unavailable"
        ) from exc
    return out_dir
...
def _strict_selection_from_bytes(value: bytes) -> dict[str, Any]:
    ...
    try:
        return validate_interactive_tree_frontier_group_selection(parsed)
    except (
        InteractiveTreeFrontierGroupSelectionError,
        TypeError,
        ValueError,
        ...
        raise StrategyError(
            "interactive-tree frontier group selection artifact failed validation"
        ) from exc
...
def _validate_inputs(inputs: object) -> dict[str, Any]:
    if not isinstance(inputs, Mapping) or any(
        ...
        raise StrategyError(
            "interactive-tree frontier group selection inputs must be an object"
        )
    actual = set(inputs)
    ...
    if missing or unexpected:
        ...
        raise StrategyError(
            "invalid interactive-tree frontier group selection inputs ("
            + "; ".join(details)
            + ")"
        )
    ...
    if isinstance(raw_node_ids, str | bytes | bytearray) or not isinstance(
        raw_node_ids,
        Sequence,
    ):
        raise StrategyError("source_node_ids must be an array")
    source_node_ids = [
        _required_text(node_id, f"source_node_ids[{index}]")
...
def _canonical_json(value: object) -> str:
    try:
        ...
        raise StrategyError(
            "interactive-tree frontier group evidence must be finite canonical JSON"
        ) from exc
