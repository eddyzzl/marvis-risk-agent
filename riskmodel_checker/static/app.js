...
function applyTheme(theme) {
  ...
  try {
    localStorage.setItem("riskmodel_checker_theme", themePreference);
  } catch (_) {
...
function restoreLayoutWidths() {
  try {
    const stored = JSON.parse(localStorage.getItem("riskmodel_checker_layout") || "{}");
    if (stored.sidebar) {
      setCssNumber(
        "--sidebar-width",
...
function restoreTheme() {
  try {
    applyTheme(localStorage.getItem("riskmodel_checker_theme") || "light");
  } catch (_) {
    applyTheme("light");
  }
}
...
function persistPetPreference(value, explicitNone = false) {
  try {
    localStorage.setItem("riskmodel_checker_pet", value);
...
function restorePetPreference() {
  try {
    const stored = localStorage.getItem("riskmodel_checker_pet");
...
function savePetPosition(left, top) {
  try {
    localStorage.setItem("riskmodel_checker_pet_position", JSON.stringify({ left, top }));
  } catch (_) {
...
function restorePetPosition() {
  try {
    const stored = JSON.parse(localStorage.getItem("riskmodel_checker_pet_position") || "{}");
...
function saveTaskListSettings() {
  try {
    localStorage.setItem("riskmodel_checker_task_list_settings", JSON.stringify({
      sort: taskSortMode,
      group: taskGroupMode,
    }));
  } catch (_) {
...
function restoreTaskListSettings() {
  try {
    const stored = JSON.parse(localStorage.getItem("riskmodel_checker_task_list_settings") || "{}");
