...
- V1.1 experience goal: let Agent use auditable historical validation metric memory to compare a new model against prior comparable models, months, channels, and scopes.
...
- V1.1 user jobs: understand whether a new validation result improved or declined compared with historical comparable models, and manage the memories used for that comparison.
...
- V1.1 memory surface: memory references should appear near Agent explanations and comparison summaries, with a path to inspect, disable, or delete entries.
...
- V1.1 components to design: memory comparison summary, memory reference list, memory inspection, memory disable/delete actions, and memory-use audit metadata.
...
- Variants and states: primary/secondary/disabled/loading/error/success buttons; empty/loading/error/success summaries; high/medium/low confidence memory comparison states.
...
- Memory comparison uncertainty: medium-confidence matches must be visually and textually marked as requiring human confirmation.
...
- Microcopy rules: no double-language eyebrow; no README-style explanatory paragraphs in every section; Agent should distinguish current evidence, historical memory, and general domain guidance.
...
- Memory constraints: memory can support explanation and comparison, but cannot alter deterministic validation metrics.
- Plugin constraints: plugin/tool/hook outputs must return structured results; platform owns Word/Excel/chart rendering.

## Open questions
- [ ] V1.1 memory management UI placement: side settings panel, task-level evidence drawer, or a dedicated memory screen.
