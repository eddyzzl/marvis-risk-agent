...
def test_univariate_tool_rejects_foreign_stale_and_nonactive_bindings(
    ...
    _settings, runner, registry, task, other, dataset, workspace, mapping = _runtime(
        tmp_path
    )
    inputs = _inputs(dataset, workspace, mapping)
...
def test_univariate_tool_rejects_explicit_sensitive_candidate(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        ToolRef("strategy", "analyze_univariate_candidates"),
        {**_inputs(dataset, workspace, mapping), "features": ["customer_id"]},
        task_id=task.id,
    )
...
def test_univariate_tool_does_not_allow_workspace_to_downgrade_sensitive_role(
    ...
    settings, runner, _registry, task, _other, dataset, workspace, _mapping = _runtime(
        tmp_path
    )
    ...
    result = runner.invoke(
        ToolRef("strategy", "analyze_univariate_candidates"),
        {
            **_inputs(dataset, downgraded_workspace, downgraded_mapping),
...
def test_univariate_tool_rechecks_source_after_analysis(
    ...
    settings, runner, registry, task, _other, dataset, workspace, mapping = _runtime(
        tmp_path
    )
...
def test_univariate_tool_preserves_concurrent_workspace_update_and_writes_nothing(
    ...
    with pytest.raises(StrategyError, match="workspace changed during analysis"):
        strategy_tools.tool_analyze_univariate_candidates(
            _inputs(dataset, workspace, mapping),
            _tool_context(settings, task),
        )
