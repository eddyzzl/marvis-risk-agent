...
def test_kernel_imported_modules_do_not_require_stdlib_strenum():
    for relative_path in [
        ...
        "riskmodel_checker/validation/results.py",
    ]:
...
def test_kernel_imported_modules_defer_annotations_at_runtime():
    for relative_path in [
        ...
        "riskmodel_checker/output/styles.py",
        "riskmodel_checker/validation/results.py",
        "riskmodel_checker/validation/reproducibility.py",
        "riskmodel_checker/validation/engine.py",
    ]:
        source = (PROJECT_ROOT / relative_path).read_text(encoding="utf-8")
...
def test_kernel_runtime_code_avoids_pep604_type_union_expressions():
    source = (PROJECT_ROOT / "riskmodel_checker/report_texts.py").read_text(encoding="utf-8")
